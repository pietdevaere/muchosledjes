...
class Display(object):
    ...
    def __init__(self, ip = '127.0.0.1', port = 5000, echo = False):
        self.ip = ip
        self.port = port
        ## open a udp socket
    ...
    def load_display(self, row_data):
        """Load an entire row data arra"""
        self.row_data = row_data[:]

    def load_array(self, array):
        strings = array_to_strings(array)
        assert len(strings) == self.rows * self.lines
        for row in range(self.rows):
            for line in range(self.lines):
                self.row_data[row][line] = strings[row*self.lines + line]
...
class RowImage(Effect):
    """Display an image on a row"""
    def __init__(self, display, image_path):
        self.d = display
        self.size = (self.d.leds_on_line, self.d.lines)
        im = Image.open(image_path).convert("L")
        im.resize(self.size, Image.ANTIALIAS)
        arr = numpy.array(im)
        self.bin_array = ['' for i in range(self.d.lines)]
        for line in range(self.size[1]):
            for pixel in arr[line]:
                if pixel < 128:
                    self.bin_array[line] += '1'
                else:
                    self.bin_array[line] += '0'
